...
def search_albums(query, limit=8):
    results = _ytmusic.search(query, filter="albums", limit=int(limit or 8))
    albums = []
    for item in results:
        thumbs = item.get("thumbnails") or []
        artists = item.get("artists") or []
...
def extract_audio(video_id):
    ...
    formats = [f for f in (info.get("formats") or []) if f.get("vcodec") == "none"]
    preferred = next((f for f in formats if str(f.get("format_id")) == "140"), None)
    chosen = preferred or max(formats, key=lambda f: f.get("abr") or 0, default=None)
    if not chosen:
        chosen = {"url": info.get("url", ""), "ext": info.get("ext", "m4a"), "abr": info.get("abr")}

    return json.dumps({
        "audioUrl": chosen.get("url", ""),
        "containerExt": chosen.get("ext", "m4a"),
        "bitrate": int(chosen.get("abr") or 0),
